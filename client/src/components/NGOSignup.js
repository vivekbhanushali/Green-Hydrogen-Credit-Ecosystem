...
const NGOSignup = ({ onLogin }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      console.log("Signing up NGO with:", formData);
      setLoadStatus(true);
      const signupResponse = await signup({ ...formData, role: 'NGO' });
      ...
      onLogin({ username: formData.username, role: 'NGO', id: loginResponse.data.user.id });
