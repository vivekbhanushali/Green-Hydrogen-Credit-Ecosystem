 ...
 */
export async function resolvePluginList(projectRoot, userConfigPluginList = []) {
 ...
 */
async function reverseTopologicalSort(projectRoot, plugins) {
    const visitedPlugins = new Map();
    const result = [];
    async function dfs(plugin) {
        const visited = visitedPlugins.get(plugin.id);
        if (visited !== undefined) {
            if (visited !== plugin) {
                throw new HardhatError(HardhatError.ERRORS.CORE.GENERAL.DUPLICATED_PLUGIN_ID, { id: plugin.id });
            }
            return;
        }
        ...
        if (plugin.dependencies !== undefined) {
            ...
            try {
                ...
                throw new HardhatError(HardhatError.ERRORS.CORE.PLUGINS.PLUGIN_DEPENDENCY_FAILED_LOAD, {
                    pluginId: plugin.id,
                }, error);
            }
...
//# sourceMappingURL=resolve-plugin-list.js.map