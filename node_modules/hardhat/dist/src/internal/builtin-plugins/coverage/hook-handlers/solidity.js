...
export default async () => ({
    preprocessProjectFileBeforeBuilding: async (context, sourceName, fsPath, fileContent, solcVersion, next) => {
        ...
        const isTestSource = fsPath.startsWith(context.config.paths.tests.solidity) ||
            fsPath.endsWith(".t.sol");
        if (context.globalOptions.coverage && !isTestSource) {
            try {
                const { source, metadata } = addStatementCoverageInstrumentation(fileContent, sourceName, solcVersion, COVERAGE_LIBRARY_PATH);
    ...
    preprocessSolcInputBeforeBuilding: async (context, solcInput, next) => {
        if (context.globalOptions.coverage) {
            ...
            const content = await readUtf8File(path.join(import.meta.dirname, "../../../../../../coverage.sol"));
