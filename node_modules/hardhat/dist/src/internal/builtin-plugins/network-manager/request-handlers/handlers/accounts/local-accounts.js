...
export class LocalAccountsHandler extends ChainId {
    ...
    async #resolveRequest(jsonRpcRequest) {
        ...
        if (jsonRpcRequest.method === "eth_sign") {
            if (params.length > 0) {
                ...
                if (address !== undefined) {
                    ...
                    return this.#createJsonRpcResponse(jsonRpcRequest.id, typed.personal.sign(data, privateKey, EXTRA_ENTROPY));
                }
            }
        }
        if (jsonRpcRequest.method === "personal_sign") {
            if (params.length > 0) {
                ...
                if (data !== undefined) {
                    if (address === undefined) {
                        throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.PERSONALSIGN_MISSING_ADDRESS_PARAM);
                    }
                    const privateKey = this.#getPrivateKeyForAddress(address);
                    return this.#createJsonRpcResponse(jsonRpcRequest.id, typed.personal.sign(data, privateKey, EXTRA_ENTROPY));
                }
        ...
        if (jsonRpcRequest.method === "eth_signTypedData_v4") {
            ...
            if (typeof data === "string") {
                try {
                    ...
                    throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.ETHSIGN_TYPED_DATA_V4_INVALID_DATA_PARAM);
                }
            }
            // if we don't manage the address, the method is forwarded
            const privateKey = this.#getPrivateKeyForAddressOrNull(address);
            if (privateKey !== null) {
                return this.#createJsonRpcResponse(jsonRpcRequest.id, signTyped(typedMessage, privateKey, EXTRA_ENTROPY));
            }
        }
        return null;
    }
    ...
    #initializePrivateKeys(localAccountsHexPrivateKeys) {
    ...
    #getPrivateKeyForAddressOrNull(address) {
        try {
    ...
    async #getSignedTransaction(transactionRequest, chainId, privateKey) {
        ...
        if ((txData.to === undefined || txData.to === null) &&
            ...
            throw new HardhatError(HardhatError.ERRORS.CORE.NETWORK.DATA_FIELD_CANNOT_BE_NULL_WITH_NULL_ADDRESS);
        }
