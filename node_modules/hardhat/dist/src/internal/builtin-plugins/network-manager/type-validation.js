...
const edrNetworkUserConfigSchema = z.object({
    ...
    initialDate: z.optional(unionType([z.string(), z.instanceof(Date)], "Expected a string or a Date")),
...
function refineEdrNetworkUserConfig(networkConfig, ctx) {
    if (networkConfig.type === "edr-simulated") {
        const { chainType = GENERIC_CHAIN_TYPE, hardfork, minGasPrice, initialBaseFeePerGas, } = networkConfig;
        if (hardfork !== undefined && !isValidHardforkName(hardfork, chainType)) {
            ctx.addIssue({
                code: z.ZodIssueCode.custom,
                path: ["hardfork"],
                message: `Invalid hardfork name ${hardfork} for chainType ${chainType}. Expected ${getHardforks(chainType).join(" | ")}.`,
            });
        }
        ...
        if (typeof networkConfig.mining?.interval === "number" ||
...
const networkUserConfigSchema = baseNetworkUserConfigSchema.superRefine(refineEdrNetworkUserConfig);
...
export async function validateNetworkConfigOverride(networkConfigOverride) {
    return validateUserConfigZodType(networkConfigOverride, networkConfigOverrideSchema);
}
