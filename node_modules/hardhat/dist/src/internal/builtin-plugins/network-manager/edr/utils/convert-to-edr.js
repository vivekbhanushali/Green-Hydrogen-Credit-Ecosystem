import { MineOrdering, FRONTIER, HOMESTEAD, DAO_FORK, TANGERINE, SPURIOUS_DRAGON, BYZANTIUM, CONSTANTINOPLE, PETERSBURG, ISTANBUL, MUIR_GLACIER, BERLIN, LONDON, ARROW_GLACIER, GRAY_GLACIER, MERGE, SHANGHAI, CANCUN, PRAGUE, BEDROCK, REGOLITH, CANYON, ECOTONE, FJORD, GRANITE, HOLOCENE, } from "@nomicfoundation/edr";
...
export function edrRpcDebugTraceToHardhat(debugTraceResult) {
    ...
    let returnValue = debugTraceResult.output ?? "0x";
    if (returnValue === "0x") {
        returnValue = "";
    }
    return {
...
export function hardhatChainDescriptorsToEdrChainOverrides(chainDescriptors, chainType) {
    return (Array.from(chainDescriptors)
        ...
        hardforkActivationOverrides: Array.from(descriptor.hardforkHistory ?? new Map()).map(([hardfork, { blockNumber, timestamp }]) => ({
