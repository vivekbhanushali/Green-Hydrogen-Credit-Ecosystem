...
import { CompilerDownloaderImplementation, CompilerPlatform, } from "./downloader.js";
async function getGlobalCompilersCacheDir() {
    const globalCompilersCacheDir = await getCacheDir();
    return path.join(globalCompilersCacheDir, "compilers-v3");
}
export async function downloadConfiguredCompilers(versions, quiet) {
    const platform = CompilerDownloaderImplementation.getCompilerPlatform();
    if (platform !== CompilerPlatform.WASM) {
        const mainCompilerDownloader = new CompilerDownloaderImplementation(platform, await getGlobalCompilersCacheDir());
    ...
    const wasmCompilerDownloader = new CompilerDownloaderImplementation(CompilerPlatform.WASM, await getGlobalCompilersCacheDir());
...
export async function getCompiler(version, { preferWasm }) {
    if (!preferWasm) {
        const platform = CompilerDownloaderImplementation.getCompilerPlatform();
        const compilerDownloader = new CompilerDownloaderImplementation(platform, await getGlobalCompilersCacheDir());
    ...
    assertHardhatInvariant(wasmCompiler !== undefined, `WASM build of solc ${version} isn't working`);
    return wasmCompiler;
}
