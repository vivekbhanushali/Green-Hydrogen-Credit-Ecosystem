...
export class ResolverImplementation {
     ...
     */
    static async create(projectRoot, readUtf8File) {
        const map = await RemappedNpmPackagesGraphImplementation.create(projectRoot);
    ...
    async resolveImport(from, importPath) {
        return this.#mutex.exclusiveRun(async () => this.#resolveImport(from, importPath));
    }
    async #resolveProjectFile(absoluteFilePath) {
        ...
        const pathValidation = await validateFsPath(this.#projectRoot, relativeFilePath);
